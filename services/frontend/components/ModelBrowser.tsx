...
export default function ModelBrowser({ onClose }: ModelBrowserProps) {
  ...
  const loadSystemInfo = async () => {
    try {
      ...
      if (info.gpu?.available && info.gpu.devices?.length > 0) {
        ...
        const totalVram = info.gpu.devices.reduce((acc: number, dev: any) => acc + dev.total_memory_gb, 0);
  ...
  const saveGlobalSettings = async () => {
    try {
      ...
      alert('Global loading settings saved!');
    } catch (error) {
          ...
          <div>
            ...
            <div className="space-y-6">
              ...
              <div className="pt-4 border-t border-gray-200">
                <h3 className="text-sm font-bold text-gray-900 mb-3">Loading Settings</h3>
                
                <div className="space-y-4">
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">Context Size</label>
                    <select 
                      value={targetContext}
                      onChange={(e) => setTargetContext(Number(e.target.value))}
                  ...
                  <div>
                    <label className="text-xs text-gray-600 block mb-1">GPU Layers</label>
                    <input 
                      type="number" 
                      value={gpuLayers}
                    ...
                    />
                    <p className="text-xs text-gray-400 mt-1">-1 = Offload all to GPU</p>
                  </div>

                  <div>
                    <label className="text-xs text-gray-600 block mb-1">Chunk Size</label>
                    <input 
                      type="number" 
                      value={nBatch}
          ...
          <div className="p-8 pb-4">
            <div className="relative max-w-2xl mx-auto">
              ...
              <input
                ...
                placeholder={activeTab === 'discover' ? "Search HuggingFace models (e.g. 'llama 3 gguf')..." : "Search installed models..."}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-6">
                ...
                {filteredInstalledModels.length === 0 && (
                  <div className="col-span-full text-center py-20 text-gray-500">
                    ...
                    <button onClick={() => setActiveTab('discover')} className="text-primary-600 hover:underline mt-2">
