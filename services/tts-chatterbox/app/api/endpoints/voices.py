...
@router.post(
    "/voices/reload",
    responses={
        200: {"description": "Voices reloaded successfully"},
        500: {"model": ErrorResponse}
    },
    summary="Reload voices from directory",
    description="Rescan the voices directory and reload voice metadata. Useful for hot-reloading voices added directly to the folder."
)
async def reload_voices():
    """Reload voices by rescanning the voices directory"""
    try:
        voice_lib = get_voice_library()
        result = voice_lib.reload_voices()
        ...
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={"error": {"message": f"Failed to reload voices: {str(e)}", "type": "voice_library_error"}}
        )
