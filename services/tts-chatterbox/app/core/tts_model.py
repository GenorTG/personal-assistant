...
async def initialize_model():
    ...
    try:
        ...
        # Use faster model loading with memory mapping if available
        # This can significantly speed up loading large models
        # Note: os is already imported at the top of the file
        os.environ.setdefault('HF_HUB_DISABLE_EXPERIMENTAL_WARNING', '1')
        
        # Set HuggingFace cache directory to use local cache
        if Config.MODEL_CACHE_DIR:
            ...
            os.environ.setdefault('HF_DATASETS_CACHE', Config.MODEL_CACHE_DIR)
        ...
        def _load_model():
            ...
            if use_multilingual:
                ...
                model = ChatterboxMultilingualTTS.from_pretrained(device=_device)
            else:
                print(f"Loading standard Chatterbox TTS model...")
                model = ChatterboxTTS.from_pretrained(device=_device)
            
            # Move model to eval mode for faster inference
            if hasattr(model, 'eval'):
                model.eval()
            
            # Pre-warm the model with a dummy inference to initialize CUDA kernels
            # This makes the first real request faster
            if Config.ENABLE_MODEL_PREWARM and _device == 'cuda' and torch.cuda.is_available():
                try:
                    print("Pre-warming model (initializing CUDA kernels)...")
                    # Just synchronize CUDA - actual warmup will happen on first request
                    # This ensures CUDA context is ready
                    torch.cuda.synchronize()
                    print("✓ Model pre-warmed")
                except Exception as e:
                    print(f"Note: Model pre-warming skipped: {e}")
            
            # Optional: Use torch.compile for faster inference (experimental)
            # This can speed up inference but adds compilation time on first use
