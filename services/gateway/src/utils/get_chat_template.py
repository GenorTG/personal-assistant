...
Based on the official script from llama.cpp:
https://github.com/ggml-org/llama.cpp/blob/master/scripts/get_chat_template.py
"""
...
def get_chat_template(model_id: str, variant: Optional[str] = None) -> str:
    """Fetch chat template from HuggingFace model repository.
    
    Args:
        model_id: HuggingFace model ID (e.g., "bartowski/Qwen2.5-7B-Instruct-GGUF")
        variant: Optional variant name if model has multiple chat templates
        
    Returns:
        Chat template string (Jinja2 format)
        
    Raises:
        Exception: If template cannot be fetched or model is gated without auth
    """
    try:
